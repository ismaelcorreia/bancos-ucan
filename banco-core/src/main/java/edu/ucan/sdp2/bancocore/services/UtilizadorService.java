...
import edu.ucan.sdp2.bancocore.dto.requisicoes.UtilizadorFotografiaRequisicaoDto;
...
//        user.setPalavraPasse(passwordEncoder.encode(user.getPalavraPasse()));
    ...
    public ResponseEntity<Resposta> activarMinhaConta(String telefone, String codigo) {
        ...
        var reqID = utilizador.getId().toString();
        var sectId = SessaoRequisicao.utilizador.getId().toString();

        if (!sectId.equals(reqID)) {
            return new Resposta<>("Somente o proprietário da conta pode activar", null).naoEncontrado();
        }
